...
function buildPrompt(type, data) {
  if (type === 'gfp') {
    ...
    const hw  = total.hw  || {};
    const cov = total.cov || {};
    return `당신은 영업 데이터 분석 전문가입니다. 아래 GFP 총괄 대시보드 데이터를 분석해서 한국어로 인사이트를 2~3문장으로 자연스럽게 서술해주세요. 현재 상태 진단, 주요 리스크, 실행 추천을 녹여서 하나의 흐름 있는 문단으로 작성하세요. JSON이나 목록 없이 문장만 출력하세요.
...
${sectionRows}`;
  ...
  if (type === 'retail') {
    ...
    return `당신은 영업 데이터 분석 전문가입니다. 아래 리테일 사업부 대시보드 데이터를 분석해서 한국어로 인사이트를 2~3문장으로 자연스럽게 서술해주세요. 현재 상태 진단, 주요 리스크, 실행 추천을 녹여서 하나의 흐름 있는 문단으로 작성하세요. JSON이나 목록 없이 문장만 출력하세요.
